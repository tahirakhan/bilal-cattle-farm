...
angular.module('app')
  .controller('FarmsCtrl',['$scope','API_URL','$http','authToken','alert','$state', function($scope,API_URL,$http,authToken,alert,$state) {
    ...
    $scope.addNewFarm = function(){
      ...
      if($scope._id == null){ 
        $http.post(API_URL+ 'api/farm',data).then(function(res){
        ...
        $http.get(API_URL+ 'api/farm/'+$scope._id).then(function(res){
          res.data.farmName = $scope.farmName;
            $http.put(API_URL+ 'api/farm/'+res.data._id,res.data).then(function(e){
    ...
    $scope.toggleActive = function(id,active){
      $http.get(API_URL+ 'api/farm/'+id).then(function(res){
    ...
    $scope.edit = function(val){
      $http.get(API_URL+ 'api/farm/'+val).then(function(res){
          $scope.farmName = res.data.farmName;
          $scope._id = res.data._id;
      });
    },
    $scope.delete = function(val){
      $http.delete(API_URL+ 'api/farm/'+val).then(function(res){
